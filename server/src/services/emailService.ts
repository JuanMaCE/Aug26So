 ...
 * Otherwise, it creates an Ethereal test account for local testing.
 */
async function getTransporter(): Promise<nodemailer.Transporter> {
  ...
  if (host && user && pass) {
    ...
    console.log(`[EmailService] Configurado transportador SMTP con host: ${host}:${port} (secure: ${secure}) para ${user}`);
  } else {
    // Modo de desarrollo: Usar cuenta de prueba Ethereal
    console.log("[EmailService] Variables SMTP no detectadas. Creando cuenta de prueba Ethereal...");
    ...
    console.log(`[EmailService] Cuenta de prueba Ethereal lista (${testAccount.user})`);
  }
...
export async function sendEmail(options: SendEmailOptions): Promise<EmailResult> {
  try {
    ...
    console.log(`[EmailService] Correo enviado exitosamente a: ${options.to} (ID: ${info.messageId})`);
    if (previewUrl) {
...
/**
 * Sends a welcome / verification email for newly registered users
 */
